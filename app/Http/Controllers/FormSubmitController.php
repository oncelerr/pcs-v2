...
class FormSubmitController extends Controller
{
    ...
    /**
     * Store form data for a candidate added by admin
     * This bypasses authentication and uses the provided user_id
     */
    public function storeForCandidate(Request $request)
    {
        try {
            // Validate request - user_id is provided in request
            $validated = $request->validate([
    ...
    /**
     * Shared logic for processing form submissions
     */
    private function processFormSubmission(array $validated)
    {
        // Save to user_information
        $userInfo = UserInformation::create([
            'user_id' => $validated['user_id'],
        ...
        if (request()->hasFile('passport_file')) {
            $file = request()->file('passport_file');
            $filePath = request()->input('passport_file_path');
            ...
            $storedPath = Storage::disk('private')->putFileAs('client_upload', $file, $filePath);
            $fileData['passport_file_name'] = $filePath;
            Log::info("Passport file saved: {$storedPath}");
        }
        ...
        if (request()->hasFile('proof_address_file')) {
            ...
            $storedPath = Storage::disk('private')->putFileAs('client_upload', $file, $filePath);
            $fileData['proof_address_file_name'] = $filePath;
            Log::info("Proof of address file saved: {$storedPath}");
        }
        ...
        if (request()->hasFile('signature_file')) {
            ...
            $storedPath = Storage::disk('private')->putFileAs('client_upload', $file, $filePath);
            $fileData['signature_file_name'] = $filePath;
            Log::info("Signature file saved: {$storedPath}");
        }
        ...
        try {
            ...
            $notificationController->notifyAdmins(
                'User Profile Completed',
                "User {$user->name} ({$user->email}) has completed their profile. {$uploadedDocumentsText}",
                [
                    'type' => 'profile_completion',
                    'data' => [
                        'user_id' => $user->id,
        ...
        try {
            ...
            if ($profileSetupStage) {
                ...
                if (!$nextStageItem) {
                    $nextStageItem = UserStageItem::where('user_id', $validated['user_id'])
                        ->where('stage_item_id', 2)
                        ->where('status', UserStageItem::STATUS_PENDING)
                        ->first();
                }

                if ($nextStageItem) {
                    $nextStageActivated = $nextStageItem->update(['status' => UserStageItem::STATUS_ACTIVE]);
                }
            }
        } catch (\Exception $e) {
        ...
        $fileUrls = [];
        if ($fileData['passport_file_name']) {
            $fileUrls['passport_url'] = $this->getSecureFileUrl($fileData['passport_file_name'], $validated['user_id'], 'passport');
        }
        if ($fileData['proof_address_file_name']) {
            $fileUrls['proof_address_url'] = $this->getSecureFileUrl($fileData['proof_address_file_name'], $validated['user_id'], 'proof_address');
        }
        if ($fileData['signature_file_name']) {
            $fileUrls['signature_url'] = $this->getSecureFileUrl($fileData['signature_file_name'], $validated['user_id'], 'signature');
        }
